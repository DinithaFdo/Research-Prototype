...
const REPORT_TEXT = {
  ...
  findings: [
    ...
    {
      label: "AOPC Faithfulness",
      value: "84.7",
      Icon: CheckCircle,
      accent: "var(--success)",
    },
    {
      label: "Homoglyph Artifacts",
      value: "14 chars",
      Icon: Eye,
      accent: "var(--accent)",
    },
...
const TOKEN_SCORES = [
  0.08, 0.92, 0.78, 0.1, 0.95, 0.71, 0.65, 0.91, 0.68, 0.87,
];
...
function DropZone({ onUpload }: { onUpload: (t: FileType) => void }) {
  ...
  const onDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      if (f)
        onUpload(
          f.name.endsWith(".wav") || f.name.endsWith(".mp3") ? "voice" : "text",
        );
    },
    [onUpload],
  );
        ...
        <div className="glass" style={{ padding: 24 }}>
          ...
          {fileType === "text" ? (
            ...
            <>
              ...
              >
                Audio Waveform + Suspicious Regions
              </div>
              <div
                style={{
                  height: 64,
...
export function InteractiveDemoPage() {
  ...
  const handleUpload = (type: FileType) => {
    setFileType(type);
    setFileName(
      type === "text" ? "sample_document.txt" : "voice_recording.wav",
    );
    setMode("uploading");
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      whiteSpace: "nowrap",
                      color:
                        i <= stepIdx ? "var(--accent)" : "var(--text-muted)",
